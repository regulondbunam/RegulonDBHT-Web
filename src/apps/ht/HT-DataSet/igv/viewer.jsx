...
export function Viewer({ id_dataset, tfs, datasetType }) {
    ...
    switch (datasetType) {
        case "TFBINDING":
            _peaksFile = `${process.env.REACT_APP_PROSSES_SERVICE}/wpds/process/ht-dataset/${id_dataset}/peaks/gff3`
            _sitesFile = `${process.env.REACT_APP_PROSSES_SERVICE}/wpds/process/ht-dataset/${id_dataset}/sites/gff3`
            if(tfs.length>0){
                _tfFiles = []
                tfs.forEach(tf => {
                    _tfFiles.push({
            ...
            break;
        case "TUS":
            _tuFile = `${process.env.REACT_APP_PROSSES_SERVICE}/wpds/process/ht-dataset/${id_dataset}/tus/gff3`
            break;
        case "TSS":
            _promoter = `/media/raw/ht_collections_web_${version}/regulondb/PromoterSet.gff3`
            _tsFile = `${process.env.REACT_APP_PROSSES_SERVICE}/wpds/process/ht-dataset/${id_dataset}/tss/gff3`
            break;
        case "TTS":
            _terminator = `/media/raw/ht_collections_web_${version}/regulondb/TerminatorSet.gff3`
            _ttFile = `${process.env.REACT_APP_PROSSES_SERVICE}/wpds/process/ht-dataset/${id_dataset}/tts/gff3`
            break;
